...
def test():
    ...
    labels = ['_CBVmap_subset_reg1_downsampled', '_TTDmap_subset_reg1_downsampled',  # TODO: _mirrored
    ...
    for i, path in enumerate(args.path):
        ...
        for batch in ds_test:
            ...
            nifph = nib.load('/share/data_zoe1/lucas/Linda_Segmentations/' + str(case_id) + '/train' + str(case_id) + '_FUCT_MAP_T_Samplespace_reg1_downsampled.nii.gz').affine
            image = np.transpose(dto.reconstructions.gtruth.interpolation.cpu().data.numpy(), (4, 3, 2, 1, 0))[:, :, :, 0, 0]
            nib.save(nib.Nifti1Image(ndi.zoom(image, zoom=(2, 2, 1)), nifph), '/data_zoe1/lucas/Linda_Segmentations/tmp/train' + str(case_id) + '_FUCT_timeOracle_CAE1.nii.gz')
