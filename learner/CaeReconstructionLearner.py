...
class CaeReconstructionLearner(Learner, CaeInference):
    ...
    def plot_epoch(self, plot, epochs):
        if self._dataloader_validation is None:
            ...
            plot.plot(epochs, [dto.loss for dto in self._metric_dtos['training']], 'r-')
            plot.plot(epochs, [dto.lesion.dc for dto in self._metric_dtos['training']], 'k-')
            plot.plot(epochs, [dto.core.dc for dto in self._metric_dtos['training']], 'c-')
            plot.plot(epochs, [dto.penu.dc for dto in self._metric_dtos['training']], 'm-')
    ...
    def visualize_epoch(self, epoch):
        ...
        for sample, time in zip(visual_samples, visual_times):
            ...
            titles = ['CBV', 'TTD', 'Lesion', 'R_Lesion',
                      ...
                      'Core', 'R_Core', 'p(1h)', 'p(3h)', 'p(5h)', 'p(10h)', 'p(16h)', 'R_Penu', 'Penumbra']
