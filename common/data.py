...
def single_data_loader3D_full_(modalities, labels, indices, batch_size, random_seed=None, shuffle=True,
                              num_workers=2, pin_memory=False, train_transform=[]):
    assert train_transform, "You must provide at least a numpy-to-torch transformation."

    # load the dataset
    dataset_train = StrokeLindaDataset2D(modalities=modalities, labels=labels,
                                         transform=transforms.Compose(train_transform),
                                         clinical='/share/data_zoe2/lucas/Linda_Segmentations/clinical_cleaned_full.csv')
...
def get_stroke_prediction_training_data(modalities, labels, train_transform, valid_transform, fold_indices, ratio,
    ...
    if split:
        return split_data_loader3D_(modalities, labels, fold_indices, batchsize, random_seed=seed,
                                   valid_size=ratio, train_transform=train_transform,
                                   valid_transform=valid_transform, num_workers=0)
    return single_data_loader3D_full_(modalities, labels, fold_indices, batchsize, random_seed=seed,
