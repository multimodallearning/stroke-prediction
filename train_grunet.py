...
from GRUnet_2 import BidirectionalSequence, tensor2index, IntegrateNet
...
class Criterion_BiNet(nn.Module):
    ...
    def forward(self, pr_core_c, pr_core_p, gt_core,
        ...
        #loss += self.weights[2] * self.multi_scale(pr_lesion_c, gt_lesion, self.dc)
        loss += self.weights[3] * self.multi_scale(pr_lesion_p, gt_lesion, self.dc)
        #loss += self.weights[4] * self.multi_scale(pr_penu_c, gt_penu, self.dc)
        loss += self.weights[5] * self.multi_scale(pr_penu_p, gt_penu, self.dc)
...
class Criterion_IntegrateNet(nn.Module):
    ...
    def multi_scale(self, input, target):
        loss = 0.0
        for scale in self.scales:
            loss += self.criterion(scale(scale(input)), scale(scale(target)))
        return loss/len(self.scales)

    def sim_edt(self, input, target):
        return self.criterion(input, target)

    def diffloss(self, displacement_grid):
        return ((displacement_grid[:, :, :, 1:, :, :] - displacement_grid[:, :, :, :-1, :, :]) ** 2).mean() +\
               ((displacement_grid[:, :, 1:, :, :, :] - displacement_grid[:, :, :-1, :, :, :]) ** 2).mean() +\
               ((displacement_grid[:, :, :, :, 1:, :] - displacement_grid[:, :, :, :, :-1, :]) ** 2).mean()

    def KLD(self, img, z_params):
        b_sz = img.size(0)
        img_sz = img.size(2) * img.size(3) * img.size(4)
        sigma = z_params[:, 0]
        mu = z_params[:, 1]
    ...
    def forward(self, pred_core, pred_lesion, pred_lesion_inv, pred_penu, gt_core, gt_lesion, gt_penu,
                displacement_grid, z_params):

        loss = [self.weights[0] * self.sim_edt(pred_core, gt_core),
                self.weights[1] * self.sim_edt(pred_lesion, gt_lesion),
                self.weights[2] * self.sim_edt(pred_lesion_inv, gt_lesion),
                self.weights[3] * self.sim_edt(pred_penu, gt_penu),
                self.weights[4] * self.diffloss(displacement_grid),
                self.weights[5] * torch.mean(torch.abs(displacement_grid)),
                self.weights[6] * self.KLD(gt_core, z_params)]

        print([str(float(l)) for l in loss])
...
def get_results_BiNet(batch, batchsize, pr_l_core, pr_l_penu, gr_l_core, gr_l_penu,
                      sequence_thresholds, t_core, idx_core):
    pr_core_p = []
    pr_lesion_p = []
    pr_penu_p = []
    off_penu_p = []
    ...
    idx_lesion = tensor2index(t_core + batch[data.KEY_GLOBAL][:, 1], sequence_thresholds)
    for b in range(batchsize):
        pr_core_p.append(pr_l_penu[b, int(idx_core[b])])
        pr_lesion_p.append(pr_l_penu[b, int(idx_lesion[b])])
        pr_penu_p.append(pr_l_penu[b, -1])
        off_penu_p.append(gr_l_penu[b, -1])

    return torch.stack(pr_core_p, dim=0).unsqueeze(1),\
           torch.stack(pr_lesion_p, dim=0).unsqueeze(1),\
           torch.stack(pr_penu_p, dim=0).unsqueeze(1),\
           torch.stack(off_penu_p, dim=0).unsqueeze(1), idx_lesion


def get_results_IntegrateNet(batch, batchsize, prs_penu, prs_penu_inverse, sequence_thresholds, t_core, idx_core):
    pr_core = []
    pr_lesion = []
    pr_lesion_inv = []
    pr_penu = []
    ...
    idx_lesion = tensor2index(t_core + batch[data.KEY_GLOBAL][:, 1], sequence_thresholds)
    for b in range(batchsize):
        pr_core.append(prs_penu[b, int(idx_core[b])])
        pr_lesion.append(prs_penu[b, int(idx_lesion[b])])
        pr_lesion_inv.append(prs_penu_inverse[b, int(idx_lesion[b])])
        pr_penu.append(prs_penu_inverse[b, -1])

    return torch.stack(pr_core, dim=0).unsqueeze(1),\
           torch.stack(pr_lesion, dim=0).unsqueeze(1),\
           torch.stack(pr_lesion_inv, dim=0).unsqueeze(1),\
...
def visualise_batch(axarr, batch, gt, pr, grid_default, grid_c, grid_p, idx_lesion,
                    n_visual_samples, sequence_length, sequence_thresholds, init_offset, factor = 3):
...
def visualise_batch_integratenet(axarr, batch, gt, prs_penu, grs_penu, prs_penu_inv, grs_penu_inv, grid_default,
                                 ...
                                 offset=0, factor=4, vmin=None, vmax=None):
    for row in range(n_visual_samples):
        titles = []
        core = gt[row, 0]
        com = np.round(ndi.center_of_mass(core)).astype(np.int)
        ...
        axarr[row*factor+offset, 0].imshow(core[com[0], :, :] < 0.5, vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('CORE')
        axarr[row*factor+offset, 1].imshow(gt[row, 1, com[0], :, :] < 0.5, vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('FUCT')
        axarr[row*factor+offset, 2].imshow(gt[row, 2, com[0], :, :] < 0.5, vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('PENU')
        for i in range(sequence_length):
            axarr[row*factor+offset, i + 3].imshow(prs_penu[row, i, com[0], :, :] < 0.5, vmin=vmin, vmax=vmax, cmap='gray')
            titles.append(get_title('Pr', row, i, batch, sequence_thresholds, idx_lesion))
        for ax, title in zip(axarr[row*factor+offset], titles):
            ax.set_title(title, verticalalignment='top')

        titles = []
        axarr[row*factor+1+offset, 0].imshow(core[com[0], :, :], vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('')
        axarr[row*factor+1+offset, 1].imshow(gt[row, 1, com[0], :, :], vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('')
        axarr[row*factor+1+offset, 2].imshow(gt[row, 2, com[0], :, :], vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('')
        for i in range(sequence_length):
            axarr[row*factor+1+offset, i + 3].imshow(prs_penu_inv[row, i, com[0], :, :], vmin=vmin, vmax=vmax, cmap='gray')
        ...
        titles = []
        axarr[row*factor+2+offset, 0].imshow(np.ones((128, 128)), vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('')
        axarr[row*factor+2+offset, 1].imshow(np.ones((128, 128)), vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('')
        axarr[row*factor+2+offset, 2].imshow(np.ones((128, 128)), vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('')
        for i in range(sequence_length):
            axarr[row*factor+2+offset, i + 3].imshow(grs_penu[row, i, com[0], :, :], vmin=vmin, vmax=vmax, cmap='gray')
            titles.append(get_title('Gr', row, i, batch, sequence_thresholds, idx_lesion))
        for ax, title in zip(axarr[row*factor+2+offset], titles):
            ax.set_title(title, verticalalignment='top')

        titles = []
        axarr[row*factor+3+offset, 0].imshow(np.ones((128, 128)), vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('')
        axarr[row*factor+3+offset, 1].imshow(np.ones((128, 128)), vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('')
        axarr[row*factor+3+offset, 2].imshow(np.ones((128, 128)), vmin=vmin, vmax=vmax, cmap='gray')
        titles.append('')
        for i in range(sequence_length):
            axarr[row*factor+3+offset, i + 3].quiver(-1 * offsets[row, i, com[0], ::16, ::16, 0].detach().cpu().numpy(),
                                                     offsets[row, i, com[0], ::16, ::16, 1].detach().cpu().numpy()[::-1, ::-1],
                                                     scale=2)
            titles.append(get_title('VecField', row, i, batch, sequence_thresholds, idx_lesion))
            '''
            axarr[row*factor+3+offset, i + 3].imshow(grs_penu_inv[row, i, com[0], :, :], vmin=vmin, vmax=vmax, cmap='gray')
            titles.append(get_title('Gr_inv', row, i, batch, sequence_thresholds, idx_lesion))
            '''
        for ax, title in zip(axarr[row*factor+3+offset], titles):
            ax.set_title(title, verticalalignment='top')
...
def visualise_batch_binet(axarr, batch, gt, pr, grid_default, grid_c, grid_p, inv_consist_core, inv_consist_penu,
    ...
    for row in range(n_visual_samples):
        ...
        for offset, grid in zip([1], [grid_p]):
            axarr[factor * row + offset + init_offset, 0].imshow(grid_default[row, 0, com[0], :, :], vmin=0, vmax=1, cmap='gray')
            titles.append('')
            axarr[factor * row + offset + init_offset, 1].imshow(grid_default[row, 0, com[0], :, :], vmin=0, vmax=1, cmap='gray')
            titles.append('')
...
def process_batch_BiNet(batch, batchsize, bi_net, criterion, sequence_length, sequence_thresholds, device):
    ...
    prs_penu, grs_penu, prs_penu_inverse, grs_penu_inverse, offsets, z_params = \
    ...
    return gt, prs_penu, grs_penu, prs_penu_inverse, grs_penu_inverse, loss, idx_lesion, offsets
...
def main(arg_path, arg_batchsize, arg_clinical, arg_commonfeature, arg_additional, arg_img2vec1,
    ...
    for epoch in range(0, arg_epochs):
        ...
        if loss_valid[-1] < min(loss_valid[:-2]):
            torch.save(bi_net, arg_path.format('valmin', 'model'))
...
def test_BiNet(arg_path, arg_batchsize, arg_clinical, arg_commonfeature, arg_additional, arg_img2vec1,
               arg_vec2vec1, arg_grunet, arg_img2vec2, arg_vec2vec2, arg_addfactor, arg_softener, arg_loss,
               arg_epochs, arg_fold, arg_validsize, arg_seed, arg_combine, arg_clinical_grunet, arg_seq_thr, model_path):
    ...
    bi_net = torch.load(model_path)

    criterion = Criterion_BiNet(arg_loss)
    loss_test = []

    f, axarr = plt.subplots(2, sequence_length + 3, figsize=(7,1))
    ...
    with torch.set_grad_enabled(is_train):
        ...
        for epoch, batch in enumerate(ds_test):
            ...
            print('Case ID', int(batch[data.KEY_CASE_ID][0]), ':', metrics.binary_measures_torch(pr[0, idx_lesion[0].squeeze().long()],
                                                                                         gt[0, 1],
                                                                                         cuda=True,
                                                                                         binary_threshold=0.5).dc)
...
def main_BiNet(arg_path, arg_batchsize, arg_clinical, arg_commonfeature, arg_additional, arg_img2vec1,
    ...
    # [0.49, 0, 0, 0, 0.49, 0, 0.02]
    criterion = Criterion_IntegrateNet([0.0001, 0, 0, 0, 0.9949, 0, 0.005], nn.MSELoss())  #[0.15, 0.03, 0.02, 0.05, 0.55, 0.1, 0.1])  #metrics.BatchDiceLoss([1.0])
    optimizer = torch.optim.Adam(params, lr=0.001)
    ...
    for epoch in range(0, arg_epochs):
        scheduler.step()
        f, axarr = plt.subplots(n_visual_samples*8, sequence_length + 3, figsize=(sequence_length + 3, n_visual_samples*8))
        loss_mean = 0
        ...
        with torch.set_grad_enabled(is_train):
            ...
            loss_train.append(loss_mean / len(ds_train))

            axarr = visualise_batch_integratenet(axarr,
                                                 batch,
                                                 gt.cpu().detach().numpy(),
                                                 prs_penu.cpu().detach().numpy(),
                                                 grs_penu.cpu().detach().numpy(),
                                                 prs_penu_inv.cpu().detach().numpy(),
                                                 grs_penu_inv.cpu().detach().numpy(),
                                                 bi_net.visual_grid.cpu().detach().numpy(),
                                                 idx_lesion,
                                                 n_visual_samples,
                                                 sequence_length,
        ...
        with torch.set_grad_enabled(is_train):
            ...
            loss_valid.append(loss_mean / len(ds_valid))
